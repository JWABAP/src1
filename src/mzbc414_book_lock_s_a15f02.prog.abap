...
*&      --> SDYN_BOOK
*&---------------------------------------------------------------------*
FORM save_new_booking  USING    p_sdyn_book TYPE sdyn_book.

  DATA : wa_book TYPE sbook.

  MOVE-CORRESPONDING p_sdyn_book TO wa_book.
  IF wa_book-class = space.
    wa_book-class = 'Y'.
  ENDIF.

  SELECT single name INTO wa_book-passname FROM scustom
...
* Implement suitable error handling here
    ...
    CALL FUNCTION 'UPDATE_SFLIGHT' IN UPDATE TASK
      EXPORTING
        iv_carrid = p_sdyn_book-carrid
        iv_connid = p_sdyn_book-connid
        iv_fldate = p_sdyn_book-fldate
*    EXCEPTIONS
*       UPDATE_FAILURE         = 1
*       FLIGHT_FULL            = 2
*       FLIGHT_NOT_FOUND       = 3
