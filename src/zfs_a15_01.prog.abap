...
START-OF-SELECTION.
  ...
  IF sy-subrc EQ 0.
    ...
    ls_fcat-coltext = 'SUMAMT'.     "SumAmount
    APPEND ls_fcat TO lt_fcat.
  ENDIF.
...
*    ENDDO.
    ...
    LOOP AT lt_fcat ASSIGNING <lf>.
      IF <lf>-fieldname CS 'WTG'.      "CS = Contain String = WTG를 포함하면 true
  ...
  CREATE OBJECT go_alv_grid
    EXPORTING
      i_parent = cl_gui_container=>screen0.       "스크린생성없이 그 위에 생성(WRITE space. 라는구문이 있어야한다.)
